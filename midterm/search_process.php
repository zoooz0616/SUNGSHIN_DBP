<?php
  ...
  $filtered_id = mysqli_real_escape_string($link, $_POST['keyword']);
  $query="select title, thumbnail_link, channelTitle, max(view_count) view_count, likes from youtube where tags LIKE '%{$filtered_id}%' group by title order by view_count desc limit 100;";
